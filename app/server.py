"""KVB Live Map — Home Assistant Supervisor Add-on HTTP server.

Ported from kvb-hafas-client's map_server.py prototype (vehicle/alert/network/
stops endpoints, background geometry prefetch) plus new pieces required by
this add-on: SQLite vehicle history (history_store.py), live + historical
stats (stats.py), a stop-search/departure-board API, and HA Ingress support.

Runs standalone too: `python3 app/server.py` with no HA around it, reading
config from env vars with the same defaults run.sh/config.yaml declare.

Ingress: HA Supervisor proxies requests to this add-on with a dynamic base
...
(e.g. debugging) and because the plan calls for handling it explicitly.
"""
